...
// ВИПРАВЛЕННЯ: Експорт класу в глобальний scope для дебагу
window.GewurzWidget = GewurzWidget;
window.CONFIG = CONFIG;

// Витягти налаштування з data-атрибутів скрипта
function getConfigFromScript() {
  // ВИПРАВЛЕННЯ: більш надійний пошук скрипт тега
  const scriptTag = 
    document.currentScript || 
    document.querySelector('script[src*="widget"]') ||
    [...document.querySelectorAll('script')].find(script => 
      script.src && script.src.includes('widget')
    );

  console.log('Found script tag:', scriptTag); // Дебаг
  ...
  console.log('Parsed config:', config); // Дебаг
  return config;
}

// Ініціалізація віджета
function initWidget() {
  console.log('Initializing widget...'); // Дебаг
...
// ВИПРАВЛЕННЯ: Множинна ініціалізація для різних станів DOM
console.log('Widget script loaded'); // Дебаг

// Спробувати ініціалізувати відразу
if (document.readyState === 'complete') {
  console.log('DOM already ready, initializing immediately');
  setTimeout(initWidget, 10);
} else if (document.readyState === 'interactive') {
  console.log('DOM interactive, initializing with timeout');
  setTimeout(initWidget, 50);
} else {
  console.log('DOM loading, waiting for ready');
  document.addEventListener('DOMContentLoaded', initWidget);
}

// Дополнительная проверка через таймер (fallback)
setTimeout(() => {
  if (!window.GewurzChatLoaded) {
